...
预期每天北京时间 02:30 运行本地 Codex 日报定时任务，并在 03:10 运行独立的精选审批发布任务。
...
```bash
sudo pmset repeat wakeorpoweron MTWRFSU 02:20:00
```
...
- 02:30 跑每日日报 Codex 任务。
- 03:10 跑精选审批发布 Codex 任务。
- 02:20 唤醒电脑，并由 LaunchAgent 保活覆盖两个任务窗口。
- Mac 尽量保持接电。
- 保持网络可用。
- 保持 Codex 登录状态正常。
