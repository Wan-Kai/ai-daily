...
- 论文解读需要按 `shuorenhua` 方法说人话，少用套话和二元对比句式。必须写成结构化小段落：`**核心结论**` 说明论文主张和要解决的问题，`**支撑证据**` 说明模型、数据、评测、实验结果或开源材料，`**我的判断**` 只写真实有用的跟进价值和风险点。
- 论文和研究摘要里，模型名、核心方法、关键数据和最终判断需要适度加粗，方便读者快速扫到重点。
